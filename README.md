...
Custody guarantees **no drain**, but the agent still runs on the host's CPU — so without more, a hostile host could submit an *in-policy* trade of its own choosing. Two roads close that, and you opt into either:

- **Verified Intents (software, any CPU).** Commit a decision rule; with `requireVerifiedIntent` the signer signs a trade only if your rule, **re-run on authenticated inputs** (signed data / inference receipts / zkTLS), produces exactly that trade — so a host can't forge a trade or fake the data it's based on. Covers checkable strategies (deterministic, or rule-over-signed-AI). Enforced in `lib/verified-intent.js` + `signer/server.js`; spec: **[docs/VERIFIED_INTENTS.md](docs/VERIFIED_INTENTS.md)**.
- **Sealed Agents (hardware, any strategy).** Run the agent in a TEE; the signer trusts only an attested enclave. For opaque strategies the signer can't re-check. Design: **[docs/SEALED_AGENTS.md](docs/SEALED_AGENTS.md)**.
